		...
		function funcionForEach(item, index){
			...
			$.ajax({
				...
				success: function(respuesta){
					...
					// TOMAMOS DE REFERENCIA LA OFERTA DEL PRODUCTO,
					//	Y SI ESTE TIENE, ENTONCES, HACEMOS USO DEL PRECIO DE OFERTA.
					if(respuesta["oferta"] == 0){
						
						console.log("Muestra precio Normal");

						precio = respuesta["precio"];

					}else{
						console.log("Muestra precio de oferta");
						precio = respuesta["precioOferta"];
						
					}
					// TOMAMOS DE REFERENCIA EL PRECIOOFERTA DEL PRODUCTO,
					//	Y SI ÉSTE EXISTE, ENTONCES, SE TOMARA DICHO VALOR, CASO
					//	CONTRARIO EL VALOR SERA EL PRECIO NORMAL
					/* 	
					if(respuesta["precioOferta"] == 0){
						
						console.log("Muestra precio Normal");

						precio = respuesta["precio"] + 2;

					}else{
						console.log("Muestra precio de oferta");
						precio = respuesta["precioOferta"] + 1;
...
/*=============================================
	BOTON PARA CAMBIAR LA CANTIDAD DEL CARRITO
=============================================*/

function mensajeErrorTransaccion($mensaje){
...
/*=============================================
	BOTON PARA CAMBIAR LA CANTIDAD DEL CARRITO,
	VALOR MAXIMO DEL STOCK.
=============================================*/

function recibirDatosStock($detallesArray){
	
	var idProducto = $('.quitarItemCarrito').attr('idProducto');

	var dataStock = new FormData();
	var respuestaProducto;

	if ($detallesArray == null) {

		dataStock.append("idProducto",idProducto);
		dataStock.append("stock","stock");
		//console.log("detalles == null");
	...
	if ($detallesArray != null) {
		...
		dataStock.append("idProducto",idProducto);
		dataStock.append("detallesArray",$detallesArray);
		dataStock.append("stock","stock");
		//console.log("detalles con datos");
	...
	$.ajax({
		...
		success: function(respuesta){
        	...
        	$(".input-number1").attr('max',respuesta["stock"])
...
/*=============================================
	OBTENEMOS EL VALOR DE LA CANTIDAD A COMPRAR Y 
	VALIDAMOS PARA QUE NO SOBREPASE LOS VALORES 
	PERMITIDOS.
=============================================*/
    ...
    if (!isNaN(currentVal)) {
        ...
        if(type == 'minus') {
            ...
            if(currentVal > input.attr('min')) {
                ...
                cantidadItem.val(currentVal - 1).change();
                cantidadItem.attr("value",(currentVal - 1));
            } 
...
$('.input-number1').on('input', function() {
	//console.log("esta tipeando");

	recibirDatosStock(null);
    ...
    name = $(this).attr('name');
    //console.log("Valor actual: " + valueCurrent);
...
$('.input-number1').on('input', function() {
	console.log("esta tipeando 2");

});

/*=============================================
	CAMBIAR EL STOCK, EN BASE A LOS DETALLES 
	DEL PRODUCTO (Color, Tamaño etc).
=============================================*/

$(document).on("change",".seleccionarDetalle", function(){

	console.log("detalls seleccionado ",$(".seleccionarDetalle").attr("id"));
	...
	for(var i = 0; i < seleccionarDetalle.length; i++){
		...
		if ($(seleccionarDetalle[i]).attr("id") == "seleccionarTalla") {

			detallesArray.push({
					"Talla":$(seleccionarDetalle[i]).val()
			});
		
		}
		
		if ($(seleccionarDetalle[i]).attr("id") == "seleccionarColor") {

			detallesArray.push({
					"Color":$(seleccionarDetalle[i]).val()
			});
	...
	//consultarStockDetalles(detallesArray);
	recibirDatosStock(detallesArray);
...
$(".agregarCarrito").click(function(){
	...
	var numCant = $('.cantidadItem').attr("value");
	var talla = "";
	var color = "";

	var agregarAlCarrito = false;

	maxValue =  parseInt($('.input-number1').attr('max'));
	console.log(numCant + " - " + maxValue);
	...
	if(tipo == "virtual"){
		...
		if ($(".seleccionarDetalle").length > 0) {
			...
			for(var i = 0; i < seleccionarDetalle.length; i++){
				...
				if($(seleccionarDetalle[i]).val() == ""){
					...
					swal({
						  title: "Debe seleccionar los detalles",
						  text: "",
						  type: "warning",
						  showCancelButton: false,
						  confirmButtonColor: "#DD6B55",
		...
		if(numCant > maxValue){
			...
			swal({
				  title: "Debe ingresar una cantidad menor a " + maxValue,
				  text: "",
				  type: "warning",
				  showCancelButton: false,
				  confirmButtonColor: "#DD6B55",
		...
		if(localStorage.getItem("listaProductos") == null){
			...
			for(var i = 0; i < listaProductos.length; i++){
				...
				if(listaProductos[i]["idProducto"] == idProducto 
					...
					&& listaProductos[i]["tipo"] == "fisico"
					&& listaProductos[i]["peso"] == peso
					&& listaProductos[i]["talla"] == talla
					&& listaProductos[i]["color"] == color
					){

					/*
					if(listaProductos[i]["cantidad"] <= numCant){
...
$("#btnCheckout").click(function(){
	...
	if(tipoArray.find(checkTipo) == "fisico"){
		...
		$(".btnPagarCulqi").attr("tipo","fisico");
		...
		$.ajax({
			...
			complete: function(){
				...
				$("#seleccionarPais option:contains(Peru)").attr("selected","true");
				$("#seleccionarPais").attr("disabled","true");
				
				incializarPais(sumaTotalPeso);
		...
		$(".btnPagarCulqi").attr("tipo","virtual");
	}
...
/*=============================================
VALIDAR EL REGISTRO DE ACTUALIZACIÓN DE DOMICILIO
=============================================*/
function registroDomicilio(){

	/*=============================================
	VALIDAR LA CIUDAD
	=============================================*/

	var ciudad = $(".textCiudad").val();
	...
	if(ciudad != ""){
		...
		if(!expresion.test(ciudad)){
			...
			$(".textCiudad").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> No se permiten números ni caracteres especiales</div>')
		...
		$(".textCiudad").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
	...
	if(region != ""){
		...
		if(!expresion.test(region)){
			...
			$(".textRegion").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> No se permiten números ni caracteres especiales</div>')
		...
		$(".textRegion").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
	...
	if(codigoPostal != ""){
		...
		if(!expresion.test(codigoPostal)){
			...
			$(".codigoPostal").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> No se permiten letras ni caracteres especiales</div>')
		...
		$(".codigoPostal").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
	...
	var direccion = $(".textDireccion").val();
	...
	if(direccion != ""){
		...
		if(!expresion.test(direccion)){
			...
			$(".textDireccion").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> Dirección: No se permiten caracteres especiales</div>')
		...
		$(".textDireccion").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
	...
	if(telefono != ""){
		...
		if(!expresion.test(telefono)){
			...
			$(".textTelefono").parent().parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> Telefono: No se permiten letras ni caracteres especiales</div>')
		...
		$(".textTelefono").parent().parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
...
/*=============================================
	BOTTON DEL MODAL PARA ACTUALIZAR LA DIRECCION DEL COMPRADOR
=============================================*/
...
$(".guardarDireccion").click(function(){
	...
	if (registroDomicilio()) {
		...
		var idUsuario = $("#btnDireccion").attr("idUsuario");
		var ciudad = $(".textCiudad").val();
		var region = $(".textRegion").val();
		var codigoPostal = $(".codigoPostal").val();
		var domicilio = $(".textDireccion").val();
		var telefono = $(".textTelefono").val();
		
		//console.log(" ",idUsuario," ",ciudad," ",region," ",codigoPostal," ",domicilio," ",telefono);	

		var formDatos = new FormData();
		formDatos.append('idUsuarioDomicilio', idUsuario);
		formDatos.append('ciudad', ciudad);
		formDatos.append('region', region);
		formDatos.append('codigoPostal', codigoPostal);
		formDatos.append('domicilio', domicilio);
		...
		$.ajax({
			...
			success: function(respuesta){
				...
				if (respuesta != null && respuesta == "ok") {
					...
					swal({
						  title: "¡OK!",
						  text: "¡Su domicilio ha sido actualizado correctamente!",
						  type:"success",
					...
					swal({
						  title: "¡ADVERTENCIA!",
						  text: "¡Llene los campos correctamente, por favor!",
						  type:"warning",
						  confirmButtonText: "Cerrar",
						  closeOnConfirm: false
						});
			...
			complete: function(){
				...
				//$("#modalDireccion input").attr('value',"");
			}
		...
		swal({
			  title: "¡ADVERTENCIA!",
			  text: "¡Verifique los campos, por favor!",
			  type:"warning",
			  confirmButtonText: "Cerrar",
			  closeOnConfirm: false
			});
...
//	 INCIALIZAMOS EL BOMBO BOX DE PAISES EN LA OPCION: PERU
//	 DE ESTA FORMA PODEMOS ESTABLECER EL PRECIO DE ENVIO NACIONAL.

function incializarPais(sumaTotalPeso){
	    ...
	    success:function(respuesta){
	    	...
	    	if(divisa == "PEN"){
		    	...
		    	for(var i = 0; i < valorItem.length; i++){
		    		...
		    		console.log((Number(conversion) * Number($(valorItem[i]).attr("valor"))).toFixed(2));
...
BOTÓN PAGAR CULQI
=============================================*/
Culqi.publicKey = 'pk_test_93BJJFemeT05ZgVx';
var datosCulqi;


$(".btnPagarCulqi").click(function(e){

	var tipo = $(this).attr("tipo");

	if(tipo == "fisico" && $("#seleccionarPais").val() == ""){

		$(".btnPagarCulqi").after('<div class="alert alert-warning">No ha seleccionado el país de envío</div>');
	...
	datos.append("metodoPago", "culqi");
	datos.append("divisaCulqi", divisa);
	datos.append("total",total);
	datos.append("totalEncriptado",totalEncriptado);
	datos.append("impuesto",impuesto);
	datos.append("envio",envio);
	...
	datosCulqi = new FormData();

	datosCulqi.append("metodoPago", "culqi");
	datosCulqi.append("divisaCulqi", divisa);
	datosCulqi.append("total",total);
	datosCulqi.append("totalEncriptado",totalEncriptado);
	datosCulqi.append("impuesto",impuesto);
	datosCulqi.append("envio",envio);
	datosCulqi.append("subtotal",subtotal);
	datosCulqi.append("tituloArray",tituloArray);
	datosCulqi.append("cantidadArray",cantidadArray);
	datosCulqi.append("valorItemArray",valorItemArray);
	datosCulqi.append("idProductoArray",idProductoArray);
...
function culqi() {
    ...
    if (Culqi.token) { // ¡Objeto Token creado exitosamente!
        ...
        //alert('Se ha creado un token:' + token + ' ,Email: ' + email);

        datosCulqi.append("token", token);
        datosCulqi.append("email", email);
        datosCulqi.append("pk", Culqi.publicKey);
         ...
         processData: false,
         	success:function(respuesta){
         		var res = JSON.parse(respuesta);
         	   	console.log(res);
         	   	var resCqiJson = JSON.parse(respuesta);

         	   	if (resCqiJson.mensaje != null) {
         	   		
         	   		console.log(JSON.parse(respuesta));
         	   		//var resError = JSON.parse(respuesta);
         	   		mensajeErrorTransaccion(resCqiJson.mensaje);

         	   	}else{

               		window.location = resCqiJson.url;

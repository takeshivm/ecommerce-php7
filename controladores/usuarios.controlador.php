...
class ControladorUsuarios{
			...
			if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["regUsuario"]) &&
			   ...
			   preg_match('/^[a-zA-Z0-9]+$/', $_POST["regPassword"])){
				...
				if($respuesta == "ok"){
					...
					$mail->setFrom('takeshi13x@gmail.com', 'Tienda Virutal E Commerce');

					$mail->addReplyTo('takeshi13x@gmail.com', 'Tienda Virutal E Commerce');
			   ...
			   preg_match('/^[a-zA-Z0-9]+$/', $_POST["ingPassword"])){
				...
				if($respuesta["email"] == $_POST["ingEmail"] && $respuesta["password"] == $encriptar){
					...
					if($respuesta["verificacion"] == 1){
						...
						$_SESSION["region"] = $respuesta["region"];
						$_SESSION["codigoPostal"] = $respuesta["codigoPostal"];
						$_SESSION["domicilio"] = $respuesta["domicilio"];
						$_SESSION["tipo_documento"] = $respuesta["tipo_documento"];
						$_SESSION["numero_documento"] = $respuesta["num_documento"];
	...
	static public function ctrRegistroRedesSociales($datos){
		...
		if($emailRepetido || $respuesta1 == "ok"){
			...
			if($respuesta2["modo"] == "facebook"){
				...
				$_SESSION["ciudad"] = $respuesta2["ciudad"];
				$_SESSION["region"] = $respuesta2["region"];
				$_SESSION["codigoPostal"] = $respuesta2["codigoPostal"];
				$_SESSION["domicilio"] = $respuesta2["domicilio"];
				$_SESSION["telefono"] = $respuesta2["telefono"];
				$_SESSION["tipo_documento"] = $respuesta2["tipo_documento"];
				$_SESSION["numero_documento"] = $respuesta2["num_documento"];
	...
	public function ctrActualizarPerfil(){
		...
		if(isset($_POST["editarNombre"])){
			...
			if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarNombre"]) &&
				...
				preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarRegion"]) &&
				preg_match('/^[0-9]+$/', $_POST["editarCodigoPostal"]) &&
				preg_match('/^[#.0-9a-zA-Z\s,-]+$/', $_POST["editarDireccion"]) &&
				...
				if (!filter_var($_POST["editarEmail"], FILTER_VALIDATE_EMAIL)) {
					...
					echo '<script> 
								...
								swal({
									  title: "¡Advertencia!",
									  text: "¡Verifique los caracteres ingresados en el campo de Email, por favor!",
									  type:"warning",
									  confirmButtonText: "Cerrar",
									  closeOnConfirm: false
									}
				...
				if ($_POST["modoUsuario"] != "" && $_POST["modoUsuario"] == "directo" ) {
					...
					$datos = array("nombre" => $_POST["editarNombre"],
								   ...
								   "domicilio" => $_POST["editarDireccion"],
								   "tipo_documento" => $_POST["tipoDocumento"],
								   "numero_documento" => $_POST["editarDocumento"],
	...
	/*=============================================
	MODIFICAR DOMICILIO DEL USUARIO QUE NO CONFIRMO EN EL REGISTRO PRINCIPAL.
	=============================================*/

	static public function ctrModificarUsuario($datos){

		if(isset($datos["idUsuario"]) && isset($datos["domicilio"])){

			if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $datos["ciudad"]) &&
				preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $datos["region"]) &&
				preg_match('/^[0-9]+$/', $datos["codigoPostal"]) &&
				preg_match('/^[#.0-9a-zA-Z\s,-]+$/', $datos["domicilio"]) &&
		...
		if(isset($_POST['mensajeContactenos'])){
			...
			preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["emailContactenos"])){
				...
				=============================================*/
					...
					$mail->setFrom('takeshi13x@gmail.com', 'Tutoriales a tu Alcance');

					$mail->addReplyTo('takeshi13x@gmail.com', 'Tutoriales a tu Alcance');
